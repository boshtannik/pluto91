...
### Что ещё есть на плате

Плата Pluto (репозиторий железа:
[carrotIndustries/pluto](https://github.com/carrotIndustries/pluto)) несёт
больше, чем МК и три кнопки:

| Периферия            | На плате                                       | Используется у нас |
|----------------------|------------------------------------------------|--------------------|
| **Дисплей** (стекло F-91W) | LCD_C, 3-mux, charge pump              | да                 |
| **Зуммер**           | P7.3 + буст-схема на катушке (~10 мГн)         | только GPIO-заглушка |
| **Подсветка**        | LED на двух GPIO (любая полярность)            | простой GPIO (без димминга) |
| **RTC_C**            | календарь + калибровка                         | нет (время от включения) |
| **Компас MAG3110**   | bit-banged I²C, питание с GPIO                 | нет                 |
| **ИК-приёмник TSOP57338** | 38 кГц, питание с GPIO                    | нет                 |
| **AES256**           | аппаратный блок в МК                           | нет                 |
| **SBW**              | 2-проводные контактные площадки в рамке        | —                   |

Компас и ИК-приёмник питаются с GPIO, поэтому в выключенном состоянии
потребляют 0 мкА. Буст-схема зуммера поднимает напряжение батареи для
громкости. Всё из колонки «не используется» — будущая работа, см. TODO в
